...
<body>
    ...
    <div class="container log-padding">
        <div class="card log-card-padd">
            <form action="assgn-to-rout.php" method="post">
                ...
                <?php $sel4 = mysqli_query($conn, "select * from routs") ?>
                <Select type="text" name="routname" class="form-control">
                    <?php while ($row4=mysqli_fetch_array($sel4)) {
                        $id = $row4['r_id'];
                        $routname = $row4['r_name']; ?>
                    <option value="<?php echo $id ?>"><?php echo $routname; ?></option>
                    <?php } ?>
                </select>
                </div>
                ...
                <div class="form-group">
                <label>Area</label>
                <?php $sel5 = mysqli_query($conn, "select * from routs") ?>
                <Select type="text" name="area" class="form-control">
                    <?php while ($row5=mysqli_fetch_array($sel5)) {
                        $id = $row5['r_id'];
                        $area = $row5['r_area']; ?>
                ...
                <div class="form-group">
                    ...
                    <button type="submit" name="assign" class="btn btn-info">Assign Rout</button>
                </div>
                <?php
